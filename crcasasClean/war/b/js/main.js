		...
		.ready(
				function() {
							...
							.click(
									function(e) {
										...
										try {
											...
											if (this.id == CR.pagesId.SEARCH)
												History.pushState(null, null,
														CR.pagesTag.SEARCH);
											if (this.id == CR.pagesId.AD)
												History.pushState(null, null,
														CR.pagesTag.AD);
											if (this.id == CR.pagesId.ADVERTISE)
												History.pushState(null, null,
														CR.pagesTag.ADVERTISE);
											if (this.id == CR.pagesId.HOME)
												History.pushState(null, null,
														CR.pagesTag.HOME);
											if (this.id == CR.pagesId.WALL)
												History.pushState(null, null,
														CR.pagesTag.WALL);
											if (this.id == CR.pagesId.ABOUT)
												History.pushState(null, null,
														CR.pagesTag.ABOUT);
											if (this.id == CR.pagesId.LANDING)
												History.pushState(null, null,
														CR.pagesTag.LANDING);
										...
										if (this.id == 'anuncio-btn') {
											...
											if (!$("#anuncio-id").val()) {
												$("#noinput-ad-alert").html(
														CR.noinput_alert_text);
											} else {
					...
					try {
						...
						console
								.log('window.addEventListener popstate not supported '
										+ ex.message);
					}
					;
...
function frmPhotoResponse(responseText, statusText, xhr, $form) {
	 ...
	 if (CR.photosCounter<=3) {
		 ...
		 CR.photosCounter = 0;
		 CR.photosRow++;
		 $("#photosFormContainer").append("<div class='row-fluid'><ul id='photosRow"+CR.photosRow+"' class='thumbnails sortable'></ul></div>");
		 $("#photosRow"+CR.photosRow).append($("#photoViewTemplate").render(obj));
...
function showResponse(responseText, statusText, xhr, $form) {
	...
	$('#publishResponse .modal-body')
			.html(
					'Su anuncio fue publicado. El # de id del anuncio es '
							+ responseText
							+ '.  Le recomendamos presionar ahora el boton Compartir de su anuncio para compartirlo en Facebook.');
...
function goToEditAd(adNumber) {
	...
	$.getJSON('http://www.crcasas.com/restlet/json/anuncio/' + adNumber
		...
		$.getJSON("http://www.crcasas.com/restlet/file/anuncio/" + adNumber
				+ "/meta" , function(data) {
...
function initUl() {
	...
	$("#myAds ul")
			.append(
					'<li class="newAdClick"><a href="#newad" data-toggle="tab">Nuevo Anuncio</a></li>');
...
function addNewUl(adNumber) {
	...
	$("#myAds ul").append(
			'<li class="editAdClick" id="' + adNumber
					+ '"><a tabindex="-1" href="#newad" data-toggle="tab">'
					+ adNumber + '</a></li>');
...
function showPayPage() {
	setFormAdSubTitle('Seleccione el tipo de anuncio');
	$('#accordionForm').hide();
	$('#payPage').fadeIn();

	$('#formBtns1Container').hide();
	$('#formBtns2Container').show();

};

function hidePayPage() {
	setFormAdSubTitle('Complete la informacion de cada seccion');
	$('#payPage').hide();
	$('#accordionForm').fadeIn();
	$('#formBtns2Container').hide();
	$('#formBtns1Container').show();
}
...
function generatePhotosInputs() {
	...
	$('.fileKey').each(
			function() {
				count++;
				inputs += '<input type="hidden" name="photosAd' + count
						+ '" value="' + $(this).html() + '" >';
...
function initNewAdTabs(tabSelector, nextSelector, prevSelector) {
								...
								.confirmModal(
										{
											...
											callback : function() {
												...
												// $("#formHeader").html('Incluir
												// Nuevo Anuncio');

												$("#myAdsTabNav").attr("href",
														"#myAdsTab")
												$(
														'#mainTab a[href="#myAdsTab"]')
														.tab('show');
	...
	$('#btnSaveAd').click(function(e) {
		...
		$('#confirmEdit').confirmModal({
			...
			callback : function() {
				...
				var queryString = $('#formAd').formSerialize();
				$.post('http://www.crcasas.com/forms/ad', queryString);
								...
								.confirmModal(
										{
											...
											callback : function() {
												$
														.ajax({
															url : 'http://www.crcasas.com/restlet/json/anuncio/'
																	+ CR.adId
																	+ '/form/delete/'
																	+ CR.userId
																	,
	...
	$('#btnSelectPause').click(
			function(e) {

				$.ajax({
					url : 'http://www.crcasas.com/restlet/json/anuncio/'
							+ CR.adId + '/form/pause/' + CR.userId
							,
					type : 'PUT',
					dataType : 'json',
					data : '',
	...
	$('#btnSelectActivate').click(
			function(e) {

				$.ajax({
					url : 'http://www.crcasas.com/restlet/json/anuncio/'
							+ CR.adId + '/form/activate/' + CR.userId
							,
					type : 'PUT',
					dataType : 'json',
					data : '',
...
function isFirstOrLastTab(tabSelector, nextSelector, prevSelector) {
	...
	if (f)
		$(prevSelector).text('Inicio');
	else
		$(prevSelector).text('Atras');
...
function get_ad(adNumber) {
	...
	$.getJSON(
			"http://www.crcasas.com/restlet/json/imagen/" + adNumber
					, function(data) {
...
function getFrontAds(adId, selector) {
	$.getJSON("http://www.crcasas.com/restlet/json/anuncio/" + adId
			, function(data) {
...
function init_ad_click(item) {
	...
	$('#mainTab button[data-target="#anuncio"]').tab('show'); // Select tab by
	// name
...
function getMyAds() {
	...
	$
			.getJSON(
					'http://www.crcasas.com/restlet/json/user/' + CR.userId
							,
					function(data) {
						...
						// if (data.length === 0) alert('Para Dele click a nuevo
						// anuncio para incluir un anuncio nuevo.');
								...
								.click(
										function() {
											...
											if (action == 'Post') {
												...
												var name = $(this).data(
														'caption');
												var caption = $(this).data(
														'address');

												postToFeed(adId, name, caption,
														description,
														'http://www.crcasas.com#!anuncio='
																+ adId,
														imageUrl);
											}
											;
											...
											if (action == 'View') {
												...
												$(
														'#mainTab button[data-target="#anuncio"]')
														.tab('show'); // Select
												// tab
												// by
												// name
...
function loadTabs() {
	...
	$('#landing').load('tabs/landing.html', function() {
