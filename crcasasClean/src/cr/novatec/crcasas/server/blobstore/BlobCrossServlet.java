...
public class BlobCrossServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res)
		...
		try {
			...
			// if (len > mb) {
			// throw new
			// RuntimeException("Sorry that file is too large. Try < 4MB file");
			// }
			...
			if (blobKey == null) {
				...
				"/crcasas/blobstoreservlet?" + "image-id=0"
				...
				image = imagesService.applyTransform(
						ImagesServiceFactory.makeCrop(0, 0, 1, 1), image);
				...
				newimageUrl = pattern.matcher(imageUrl).replaceFirst(
						"127.0.0.1");
			...
			res.sendRedirect("/crcasas/blobcross?" + "image-id=0");

			DataBaseServicesImpl db = new DataBaseServicesImpl();
			db.sentEmail("BlobstoreServlet exception", e.getMessage());
			db.saveError(new Errors(new Date(), e.getMessage(), e
					.getLocalizedMessage(), "BlobStoreServlet",
	...
	public JSONObject getBlobInfo(ImageDataBase record) throws JSONException {
		...
		long size = record.getSize().longValue();
		...
		j.put("size", size);
		j.put("url", url);
		j.put("thumbnail_url", url + "=s144");

		j.put("delete_url", "/restlet/file/" + record.getKey()
				+ "/meta");
		j.put("delete_type", "DELETE");
		j.put("key", record.getKey());
